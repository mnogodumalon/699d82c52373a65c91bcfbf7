...
import type { TaeglicherCheckIn, TrackingEintraege, Gewohnheiten, CreateTaeglicherCheckIn, CreateTrackingEintraege, CreateGewohnheiten } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getTrackingEintraege(): Promise<TrackingEintraege[]> {
    ...
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as TrackingEintraege[];
    return enrichLookupFields(records, 'tracking_eintraege');
  }
  static async getTrackingEintraegeEntry(id: string): Promise<TrackingEintraege | undefined> {
    ...
    const record = { record_id: data.id, ...data } as TrackingEintraege;
    return enrichLookupFields([record], 'tracking_eintraege')[0];
  }
  static async createTrackingEintraegeEntry(fields: CreateTrackingEintraege) {
    return callApi('POST', `/apps/${APP_IDS.TRACKING_EINTRAEGE}/records`, { fields: cleanFieldsForApi(fields as any, 'tracking_eintraege') });
  }
  static async updateTrackingEintraegeEntry(id: string, fields: Partial<CreateTrackingEintraege>) {
    return callApi('PATCH', `/apps/${APP_IDS.TRACKING_EINTRAEGE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'tracking_eintraege') });
  }
  ...
  static async getGewohnheiten(): Promise<Gewohnheiten[]> {
    ...
    return enrichLookupFields(records, 'gewohnheiten');
  }
  static async getGewohnheitenEntry(id: string): Promise<Gewohnheiten | undefined> {
    ...
    const record = { record_id: data.id, ...data } as Gewohnheiten;
    return enrichLookupFields([record], 'gewohnheiten')[0];
  }
  static async createGewohnheitenEntry(fields: CreateGewohnheiten) {
    return callApi('POST', `/apps/${APP_IDS.GEWOHNHEITEN}/records`, { fields: cleanFieldsForApi(fields as any, 'gewohnheiten') });
  }
  static async updateGewohnheitenEntry(id: string, fields: Partial<CreateGewohnheiten>) {
    return callApi('PATCH', `/apps/${APP_IDS.GEWOHNHEITEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'gewohnheiten') });
  }
