...
export function GewohnheitenDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = false }: GewohnheitenDialogProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      await onSubmit(fields as Gewohnheiten['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "gewohnheit_name": string | null, // Name der Gewohnheit\n  "beschreibung": string | null, // Beschreibung\n  "kategorie": "gesundheit" | "fitness" | "ernaehrung" | "produktivitaet" | "persoenliche_entwicklung" | "soziales" | "finanzen" | "kreativitaet" | null, // Kategorie\n  "ziel_haeufigkeit": "taeglich" | "mehrmals_woche" | "woechentlich" | "mehrmals_monat" | "monatlich" | null, // Ziel-Häufigkeit\n  "startdatum": string | null, // YYYY-MM-DD // Startdatum\n  "aktiv": boolean | null, // Gewohnheit ist aktiv\n}`;
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{defaultValues ? 'Gewohnheiten bearbeiten' : 'Gewohnheiten hinzufügen'}</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={v => setFields(f => ({ ...f, kategorie: v === 'none' ? undefined : v as 'gesundheit' | 'fitness' | 'ernaehrung' | 'produktivitaet' | 'persoenliche_entwicklung' | 'soziales' | 'finanzen' | 'kreativitaet' | 'sonstiges' }))}
            >
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={v => setFields(f => ({ ...f, ziel_haeufigkeit: v === 'none' ? undefined : v as 'taeglich' | 'mehrmals_woche' | 'woechentlich' | 'mehrmals_monat' | 'monatlich' }))}
            >
